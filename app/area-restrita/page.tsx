  ...
  return (
    <main className="min-h-screen bg-white flex flex-col">
      ...
      <section className="bg-primary pt-32 pb-20 px-4 relative overflow-hidden">
        {/* Background elements */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-white/10 blur-[100px] rounded-full tranneutral-x-1/2 -tranneutral-y-1/2 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-primary/20 blur-[80px] rounded-full -tranneutral-x-1/2 tranneutral-y-1/2 pointer-events-none"></div>
            ...
            {accessLinks.map((item, idx) => (
              <a
                ...
                className={`group relative p-8 sm:p-10 rounded-2xl border transition-all duration-300 flex flex-col sm:flex-row gap-6 items-start sm:items-center cursor-pointer ${
                  item.primary 
                    ? "bg-primary border-primary hover:shadow-xl hover:shadow-primary/30 hover:-tranneutral-y-1" 
                    : "bg-white border-neutral-200 hover:border-primary/30 hover:shadow-xl hover:-tranneutral-y-1"
                }`}
              >
                <div className={`flex-shrink-0 w-16 h-16 rounded-xl flex items-center justify-center transition-colors shadow-sm ${
                  item.primary ? "bg-white/20 text-white" : "bg-white text-neutral-700 group-hover:bg-primary/10 group-hover:text-primary"
                }`}>
                ...
                <div className="flex-grow">
                  ...
                  <p className={`font-medium text-sm sm:text-base leading-relaxed ${item.primary ? "text-white/80" : "text-neutral-500"}`}>
                    {item.description}
                  </p>
                </div>
                
                <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center transition-transform group-hover:tranneutral-x-2 ${
                  item.primary ? "bg-white/20 text-white" : "bg-white text-neutral-400 group-hover:bg-primary/10 group-hover:text-primary"
                }`}>
